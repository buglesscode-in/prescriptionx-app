...
export function TemplateModal({
  ...
  // --- Handlers ---
  const handleMedicationChange = (id: string, field: keyof MedicationData, value: string) => {
    if (field === 'regimen') {
  ...
  const isSaveDisabled = templateName.trim() === '' || validMedsCount === 0 || isLoading;
    ...
    <Dialog open={isOpen} onOpenChange={handleCancel}>
      <DialogContent className="sm:max-w-[1000px] max-h-[85vh] overflow-y-auto p-8">
        <DialogHeader>
          ...
          <DialogTitle>
            {editingTemplate ? 'Edit Medication Template' : 'Create New Medication Template'}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="flex justify-end pt-4">
          <Button variant="outline" size="sm" onClick={addMedicationRow} disabled={isLoading}>
                  ...
                  <TableRow key={med.id}>
                    ...
                    <TableCell>
                      <Input
                        ...
                        value={med.regimen}
                        onChange={(e) => handleMedicationChange(med.id, 'regimen', e.target.value)}
                        disabled={isLoading}
                      />
                    </TableCell>
                    <TableCell>
                      <Select
                        value={med.mealTime}
                        onValueChange={(value: 'Before Meal' | 'After Meal' | 'With Meal') =>
                    ...
                    <TableCell>
                      <Select
                        value={med.frequency}
                        onValueChange={(value: 'Daily' | 'Weekly' | 'Twice a day' | 'As needed') =>
                    ...
                    <TableCell>
                      <Input
                        ...
                        value={med.remarks}
                        onChange={(e) => handleMedicationChange(med.id, 'remarks', e.target.value)}
                        disabled={isLoading}
                      />
                    </TableCell>
                    <TableCell>
        ...
        <DialogFooter className="mt-6">
          ...
          <Button type="button" onClick={handleSaveClick} disabled={isSaveDisabled}>
            {isLoading ? (
              <>
